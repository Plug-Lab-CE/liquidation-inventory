...
export async function PATCH(req: Request, { params }: Params) {
  ...
  if (!isDatabaseConfigured()) {
    const existing = memoryGetItem(id);
    if (!existing) {
    ...
    memoryUpdateItem(id, updates as Partial<MemoryItem>);
  ...
  if ("salePrice" in data && data.salePrice !== undefined) {
    ...
    const sale = recomputeSalePrice(
      existing.unitRetail != null ? String(existing.unitRetail) : null,
      discountPercent,
    );
    updates.salePrice = sale;
  }
